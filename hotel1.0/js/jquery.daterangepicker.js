...
(function (factory) {
		if (typeof define === 'function' && define.amd) {              //typeof 可以用来检测给定变量的数据类型
...
}(function ($, moment)
{
	...
	$.fn.dateRangePicker = function(opt)
	{
		...
		//show one month on mobile devices 在移动设备上显示一个月
		...
		function init_datepicker()
		{
			...
			setTimeout(function()
			{
				...
				box.find('.month-wrapper').width($(window).width());
				initiated = true;
			},0);
		...
		// Return the date picker wrapper element   返回日期选择器包装元素
		...
		function updateSelectableRange(day)
		{
			...
			if (opt.start && !opt.end)
			{
				...
				//扩展节假日start
				//添加节日
	   /*         var holidays = [["2016/1/1", "元旦"], ["2016/2/7", "除夕"], ["2016/2/8", "春节"], ["2016/4/4", "清明"], ["2016/5/1", "劳动"], ["2016/6/9", "端午"], ["2016/9/15", "中秋"], ["2016/10/1", "国庆"]];
				var myTime=new Date(time);
				var s=myTime.getFullYear()+"/"+(myTime.getMonth()+1)+"/"+myTime.getDate();	
				for(var i=0;i<holidays.length;i++){
				  if(s==holidays[i][0]){
					 return '<div><span style="font-weight:bold">'+date+'</span><div class="on">'+s+'</div></div>';	
				  }
				}*/
				//扩展节假日end
		...
		//alert(box.find('.start-day').html(getDateString(new Date(parseInt(opt.start)))));

		function showSelectedInfo(forceValid,silent)            //点击执行的
		{
			
	      // alert(countDays(opt.end, opt.start)+1)    //测试用的
			...
			if (opt.start)
			{
				...
				//$(".dcb").html(getDateString(new Date(parseInt(opt.start))));
				
				$(".start-and-end").text(2);    //扩展几晚2015-10-20    
			}
			if (opt.end)
			{
				box.find('.end-day').html(getDateString(new Date(parseInt(opt.end))));
				//$(".dcb").html(getDateString(new Date(parseInt(opt.end))));
			...
			{
				box.find('.selected-days').show().find('.selected-days-num').html(countDays(opt.end, opt.start)+0);   ///几晚
				
				$(".start-and-end").text(countDays(opt.end, opt.start)+0);        //扩展几晚2015-10-20    
	            //$(".add i").html(box.find('.selected-days').show().find('.selected-days-num').html(countDays(opt.end, opt.start)+1));
				box.find('.apply-btn').removeClass('disabled');
				var dateRange = getDateString(new Date(opt.start))+ opt.separator +getDateString(new Date(opt.end));
				opt.setValue.call(selfDom,dateRange, getDateString(new Date(opt.start)), getDateString(new Date(opt.end)));
				if (initiated && !silent)
			...
			//$(".start-and-end").text(countDays(opt.end, opt.start)+1);   //扩展几晚2015-10-20     
		...
		function showSelectedDays()
		{
			...
			box.find('.day').each(function()
			{
				var time = parseInt($(this).attr('time')),
			   	...
			   	$(this).find("on").text("元旦");
			    
					
			//扩展节假日start
	       var holidays = [["2016/1/1", "元旦"], ["2016/2/7", "除夕"], ["2016/2/8", "春节"],["2016/2/14", "情人节"],["2016/2/22", "元宵节"], ["2016/4/4", "清明节"], ["2016/5/1", "劳动节"], ["2016/6/9", "端午节"], ["2016/9/15", "中秋节"], ["2016/10/1", "国庆节"]
         ];
			var myTime=new Date(time);
			var s=myTime.getFullYear()+"/"+(myTime.getMonth()+1)+"/"+myTime.getDate();
			for(var i=0;i<holidays.length;i++){
			...
			//扩展节假日end
				...
				//add first-date-selected class name to the first date selected  将第一个日期选择的类名称添加到选定的第一个日期
				if ( opt.start && moment(start).format('YYYY-MM-DD') == moment(time).format('YYYY-MM-DD'))
				{   
					$(this).addClass('first-date-selected');
					$(this).find('.on').text('入住');    //插件扩展  2015-10-15 Dabin
				...
				else
				{
					...
					$(this).find('.on').text('');       //插件扩展  2015-10-15 Dabin
				
				}
				//add last-date-selected  添加最后一个日期
				if ( opt.end && moment(end).format('YYYY-MM-DD') == moment(time).format('YYYY-MM-DD') )
				{
					$(this).addClass('last-date-selected');
					$(this).find('.on').text('离店');    //插件扩展  2015-10-15 Dabin
				...
				//插件扩展添加今天  到底是什么逻辑？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？
				if($(this).hasClass('real-today')){
				     $(".real-today").find('.on').text('入住');   
				}else if($(this).hasClass('first-date-selected')){
					 $(".real-today").find('.on').text('今天');    //插件扩展  2015-10-15 Dabin
				}
		...
		function createDom()
		{
			...
			var html='<div id="pop" style="width:100%;height:100%;position:fixed;z-index:998;bottom:0;background:rgba(0,0,0,.5);"></div>'
			...
			if (opt.showTopbar)
			{
				...
				else
				{
					html += '<div class="normal-top">\
							<p class="l-position">取消<em></em></p><b class="start-day">...</b>'+'入住，';
					if ( ! opt.singleDate ) {
						/*html += ' <span class="separator-day">'+opt.separator+'</span> <b class="end-day">...</b> <i class="selected-days">共<span class="selected-days-num">3</span> '+lang('days')+'</i>'*/
						html += '<i class="selected-days">共<span class="selected-days-num"></span> '+lang('days')+'</i>'
					}
					html += '</div>';
				}
				...
				html += '<div class="error-top">error</div>\
						...
						<p class="apply-btn disabled'+ getApplyBtnClass() +'"><em></em>'+lang('apply')+'</p>';
				html += '</div>'
			}

			html += '<div class="month-wrapper">'
				+'<table class="month1" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;"><span class="prev"><i class="iconfont">&#xe600;</i></span></th><th colspan="5" class="month-name">January, 2011</th><th style="width:27px;">' + (opt.singleDate || !opt.stickyMonths ? '<span class="next"><i class="iconfont" style="font-size: 1.05rem;">&#xe617;</i></span>': '') + '</th></tr><tr class="week-name">'+getWeekHead()+'</thead><tbody></tbody></table>';
